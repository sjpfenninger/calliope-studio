...
model — `calliope_studio.results.geo` over what `calliope.read_yaml` produced — because
...
It produces the same GeoJSON shape `calliope_studio.results.geo` does, so one map
...
from calliope_studio.modeldef.entities import merged_section, resolved_techs
