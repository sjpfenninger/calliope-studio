...
`uvicorn calliope_studio.server.app:app --reload` works during development, taking
its workspace from the `CALLIOPE_STUDIO_WORKSPACE` environment variable.
"""
...
from calliope_studio.server.routes import api_router
from calliope_studio.server.storage import LocalStorage

WORKSPACE_ENV_VAR = "CALLIOPE_STUDIO_WORKSPACE"
...
def create_app(
    ...
    Args:
        workspace: Model definition folder or solved `.nc` file to open on
            start. Falls back to `$CALLIOPE_STUDIO_WORKSPACE`, then the working
            directory.
    ...
    app = FastAPI(title="Calliope Studio", version=_version())
    ...
    # `calliope-studio results.nc` opens a solved model directly, with no model
...
def _version() -> str:
    from calliope_studio import __version__
...
def __getattr__(name: str):
    ...
    `uvicorn calliope_studio.server.app:app` resolves this attribute when it starts
    serving, by which point the CLI has set `$CALLIOPE_STUDIO_WORKSPACE`. Building it
