...
const timeseriesVariables = computed(() => store.catalog?.variables.timeseries ?? []);
...
<template>
  <div class="flex min-h-0 flex-1" data-testid="run-results">
    ...
    <main class="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-2">
      <p v-if="store.error" class="rounded-sm bg-danger-soft p-2 text-sm text-danger-text">
        {{ store.error }}
      </p>

      <section
        v-if="store.hasGeography"
        class="rounded-sm border border-border bg-surface"
      >
        <header class="flex h-7 items-center gap-2 border-b border-border-subtle px-2">
          <span class="text-sm font-medium">{{ store.variableStatic }} by node</span>
          <div class="flex-1" />
      ...
      <section class="rounded-sm border border-border bg-surface">
        ...
        >
          <Select v-model="store.variableTimeseries">
            ...
            <SelectContent>
              <SelectItem v-for="name in timeseriesVariables" :key="name" :value="name">
                {{ name }}
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <section class="rounded-sm border border-border bg-surface">
        <header class="flex h-9 items-center gap-1.5 border-b border-border-subtle px-2">
          <Select v-model="store.variableStatic">
            <SelectTrigger size="sm" class="h-7 w-48" data-testid="static-variable">
              <SelectValue placeholder="Variable" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem v-for="name in staticVariables" :key="name" :value="name">
                {{ name }}
              </SelectItem>
            </SelectContent>
          </Select>
